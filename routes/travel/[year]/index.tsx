    ...
    <div class="max-w-screen-md px-4 mx-auto space-y-12 md:px-0">
      ...
      <section className="space-y-4">
        {timeline.length > 0 && (
          <header className="grid grid-cols-3">
            <div className="text-left">
              {previousYear && (
                <a href={`/travel/${previousYear}`}>{`← ${previousYear}`}</a>
              )}
            </div>
            ...
            <div className="text-right">
              {nextYear && (
                <a href={`/travel/${nextYear}`}>{`${nextYear} →`}</a>
              )}
            </div>
          </header>
        )}
        <Timeline
          ...
          timeline={timeline.filter(({ type }) => type === "travel")}
          query={query}
          hasMoreHref={previousYear ? `/travel/${previousYear}` : undefined}
